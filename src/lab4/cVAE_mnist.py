...
if __name__ == "__main__":
    ...
    y1 = F.one_hot(label1.unsqueeze(0), num_classes=n_classes).float().to(device).squeeze(0)
    y2 = F.one_hot(label2.unsqueeze(0), num_classes=n_classes).float().to(device).squeeze(0)
    ...
    print("Loaded two MNIST images for interpolation example. cVAE model is trained and loaded.")